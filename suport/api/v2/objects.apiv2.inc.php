...
class ApiV2
{
    ...
    function ChatsList()
    {
        ...
        $result = queryDB(true,"SELECT * FROM `".DB_PREFIX.DATABASE_CHAT_ARCHIVE."` ".$sql_where." ORDER BY `chat_id` ASC ".$sql_limit.";");
    ...
    function TicketEditorAssign()
    {
        ...
        if($editor = $this->CreateFromJSON($params,json_decode($_POST["p_data"]),$editor,"TicketEditor"))
        {
            ...
            if($Ticket->Load())
            {
                ...
                if(isset($INTERNAL[$editor->Editor]))
                {
                    if(!empty($editor->GroupId) && in_array($editor->GroupId,$INTERNAL[$editor->Editor]->GetGroupList(true)))
                    {
                        $editor->Save();
    ...
    static function GetGeneralDefinitions()
    {
        ...
        $general["Authentication"]["Password"] = array("Type"=>"string","Required"=>true,"Description"=>"API Authentication Password","Example"=>"password");
    ...
    static function GetObjectFields($_objName)
    {
        ...
        if($_objName=="Chat")
            return array(
                "ChatId"=>array("Input"=>false,"Output"=>true,"Type"=>"int","Description"=>"The serial chat ID","Required"=>true,"Example"=>"11123"),
        ...
        if($_objName=="TicketMessage")
            return array(
                "Id"=>array("Input"=>false,"Output"=>true,"Type"=>"string","Description"=>"Message ID<br><br>First message's ID equals ticket ID.","Required"=>false,"Example"=>"11701","MultilineDescription"=>true),
