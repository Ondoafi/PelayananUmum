...
function setOperator($util=0,$fromCookie=null,$result=true,$_allowBots=false,$_requireBot=false)
{
	global $CONFIG,$INTERNAL,$GROUPS,$USER,$INTLIST,$INTBUSY,$WELCOME_MANAGER;
	$INTLIST = array();
	$INTBUSY = 0;
	$backup_target = null;
	$fromDepartment = $fromDepartmentBusy = false;
    $predefined = getPredefinedOperator($fromCookie,$_allowBots,$_requireBot);
	...
	{
		if(!$internal->Deactivated && !$internal->MobileSleep($USER->Browsers[0]) && $internal->Status != USER_STATUS_OFFLINE && ($_allowBots || !$internal->IsBot) && (!$_requireBot || $internal->IsBot))
		{
			...
			{
                $intstatus = $internal->GetMaxChatAmountStatus($GROUPS[$USER->Browsers[0]->DesiredChatGroup]);
				if($WELCOME_MANAGER && $internal->IsBot && $internal->WelcomeManager)
					$USER->Browsers[0]->DesiredChatPartner = $sessId;
				if($intstatus == USER_STATUS_ONLINE && ($internal->LastChatAllocation < (time()-15) || $internal->IsBot))
                {
					$group_available[$sessId] = GROUP_STATUS_AVAILABLE;
                }
				elseif($intstatus == USER_STATUS_BUSY || ($internal->LastChatAllocation >= (time()-15) && !$internal->IsBot))
				{
                    ...
                    {
                        if($USER->Browsers[0]->DesiredChatPartner != $predefined)
                        {
                            $USER->Browsers[0]->DesiredChatPartner = $predefined;
                            $USER->Browsers[0]->SetTargetOperator($predefined);
                        }
						return;
                    }
				}
	...
	if(isset($group_chats) && is_array($group_chats) && isset($fromDepartment) && $fromDepartment)
		foreach($group_chats as $sessId => $amount)
			if(($group_available[$sessId] == GROUP_STATUS_AVAILABLE && $amount <= $util) || ((!empty($USER->Browsers[0]->Forward) && $USER->Browsers[0]->Forward->Processed) && isset($predefined) && $sessId == $predefined))
		...
		if(is_array($available_internals))
		{
			...
			else
			{
				...
				else
				{
                    ...
                    {
                        $available_internals_prio[$sessId] = $GROUPS[$USER->Browsers[0]->DesiredChatGroup]->GetChatPriority($sessId);
                        $maxp = max($maxp,$available_internals_prio[$sessId]);
                    }

                    if($maxp > 0)
                    {
                        arsort($available_internals_prio);
                        $matching_internal = key($available_internals_prio);
                    }
                    else
                    {
    ...
    if($CONFIG["gl_alloc_mode"] == ALLOCATION_MODE_ALL && (!empty($CONFIG["gl_iada"]) || !empty($CONFIG["gl_imda"])))
    {
        if(!empty($USER->Browsers[0]->DesiredChatPartner) && count($USER->ChatRequests)>0 && !$INTERNAL[$USER->Browsers[0]->DesiredChatPartner]->IsBot)
        {
            if((!empty($CONFIG["gl_iada"]) && !empty($USER->ChatRequests[0]->EventActionId)) || (!empty($CONFIG["gl_imda"]) && empty($USER->ChatRequests[0]->EventActionId)))
            {
                $result = false;
                $USER->Browsers[0]->DesiredChatPartner = null;
            }
...
{
	$result = queryDB(true,"SELECT count(id) as rating_count FROM `".DB_PREFIX.DATABASE_RATINGS."` WHERE time>".DBManager::RealEscape(time()-86400)." AND ip='".DBManager::RealEscape(getIP())."';");
	if($result)
	{
		$row = DBManager::FetchArray($result);
