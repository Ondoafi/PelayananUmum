...
function lz_chat_pop_out()
{
    ...
    var name = (lz_chat_get_input_value(111) != "") ? lz_global_base64_url_encode(lz_chat_get_input_value(111)) : "";
    var email = (lz_chat_get_input_value(112) != "") ? lz_global_base64_url_encode(lz_chat_get_input_value(112)) : "";
    var params = lz_tracking_chat_params(name,email,operator,group,false);
...
function lz_chat_switch_options(_cancel)
{
	if(!_cancel)
	{
		...
		else
		{
            ...
            lz_chat_init_data_change(null,lz_global_trim(document.getElementById('lz_chat_overlay_options_transcript_email').value));
            lz_session.Save();
		}
...
{
	if(_name==null)
		_name = lz_chat_get_input_value(111);
	if(_email==null)
...
function lz_chat_message(_msg,_trans)
{
    ...
    if(transInto != "" && lz_session.TransFrom!=null && lz_session.TransFrom!="" && lz_session.TransFrom.toUpperCase() != transInto.toUpperCase() && _msg==null)
    {
...
function lz_chat_set_state_bar(_visible)
{
    if(_visible)
    {
        document.getElementById("lz_chat_content_box").style.top="28px";
...
function lz_chat_prepare_data_form()
{
    ...
    document.getElementById('lz_chat_overlay_data_form_ok_button').style.display = (lz_mode_change_details) ? "none" : "";
    document.getElementById('lz_chat_overlay_data_form_ok_button').innerHTML = (lz_chat_data.CurrentApplication!="chat" || lz_mode_create_ticket) ? lz_text_send_message : lz_text_send_message;
    ...
    document.getElementById("lz_chat_overlay_data_form_ok_button").style.left = (document.getElementById("lz_chat_overlay_data_form_cancel_button").style.display=='none') ? "15px" : "";
    document.getElementById("lz_chat_ticket_received").style.display = (lz_mode_ticket_feedback) ? "" : "none";
...
function lz_chat_poll_parameters() 
{
    ...
    if(lz_change_transcript)
    {
        lz_change_transcript = false;
        params += "&tc=" + lz_session.Transcript.toString();
    }
...
function lz_chat_mail_callback(_received)
{
	...
	document.getElementById('lz_chat_ticket_received').style.display = (_received) ? "" : "none";
	document.getElementById('lz_chat_ticket_flood').style.display = (!_received) ? "" : "none";
	if(_received)
	{
...
function lz_chat_set_element_width()
{
	for(var i = 0;i<document.getElementById("lz_chat_content_box").childNodes.length;i++)
		if(document.getElementById("lz_chat_content_box").childNodes[i].tagName.toLowerCase() == "div")
			document.getElementById("lz_chat_content_box").childNodes[i].style.width = (lz_chat_scrolled || document.getElementById("lz_chat_content_box").scrollHeight > document.getElementById("lz_chat_content_box").clientHeight) ? "252px" : "267px";
}
...
function lz_chat_load_input_values()
{
    for(var i = 0;i< lz_chat_data.InputFieldIndices.length;i++)
    {
        ...
        if(document.getElementById("lz_form_" + findex) != null)
        {
            ...
            {
                if(findex != 114 && document.getElementsByName("form_" + findex)[0].value=="")
