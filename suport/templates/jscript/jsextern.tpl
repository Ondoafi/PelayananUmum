...
function lz_chat_set_translation(_activeId,_from,_into)
{
    ...
    if(lz_chat_data.TranslationActive != _activeId)
    {
        ...
        lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_translation_target_language').disabled = (_from == null);
        lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_translation_service_active').checked = (_from != null);
    }
    else
        return;
    ...
    if(lz_chat_data.TranslateFrom != _from && _from != null)
    {
        ...
        for(var i=0;i<lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_translation_target_language').options.length;i++)
        {
            if(lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_translation_target_language').options[i].value==_from)
                lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_translation_target_language').selectedIndex = i;
        }
    }
...
function lz_chat_chat_resize_detect(_on)
{
    ...
    else if(false && !_on && lz_resize_interval != null)
    {
        setTimeout("lz_chat_chat_resize_input()",500);
        clearInterval(lz_resize_interval);
        lz_resize_interval = null;
    }
}
...
function lz_chat_chat_resize_input()
{
    try
    {
        var tbheight = parseInt(lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_text').style.height.replace("px",""));
        lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_floor').style.height = tbheight+30+"px";
        lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_main').style.bottom = tbheight+30+"px";
    }
    catch(ex)
    {
    }
...
function lz_chat_rating_drop_down()
{
    if(lz_chat_data.Status.Status < lz_chat_data.STATUS_STOPPED)
        if(!lz_chat_data.Rated)
            if(!lz_chat_is_dropdown_open())
                lz_chat_switch_feedback();
}
...
function lz_chat_startup() 
{
	if(lz_chat_data.Groups == null)
	{
        ...
        lz_chat_data.Groups.StatusIcon = !<!--small-->;
...
function lz_chat_send_rate(_qualification,_politeness,_comment)
{
	if(lz_chat_data.InternalUser.Id.length > 0)
	{
		...
		lz_chat_feedback_callback(lz_chat_data.Language.WaitForRepresentative,true);
}

function lz_chat_send_rate_callback(_success)
{
	if(_success)
		lz_chat_feedback_callback(lz_chat_data.Language.RateSuccess);
	else
		lz_chat_feedback_callback(lz_chat_data.Language.RateMax);
}
    ...
    if(_action == "send_mail")
    {
        ...
        for(var i=0;i<10;i++)
        {
            if(lz_chat_data.InputFieldValues[i].Active && lz_chat_data.InputFieldValues[i].Index.toString().length==1 && lz_chat_data.InputFieldValues[i].Value.toString().length>0)
            {
                ...
                {
                    fd.append("p_cf" + lz_chat_data.InputFieldValues[i].Index, lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','').document.getElementsByName("form_" + lz_chat_data.InputFieldValues[i].Index)[0].files[0]);
...
function lz_chat_get_post_values(_action, _groups, _params)
{
	...
	if(_action == "listen" || _action == "shout")
	{
		...
		if(lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_transcript_email') != null)
			values += "&p_tc_email=" + lz_global_base64_url_encode(lz_global_trim(lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_transcript_email').value));
		if(lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_send_chat_transcript') != null && '<!--transcript_option_display-->' != 'none' && !lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_send_chat_transcript').checked)
...
function lz_chat_login(_groupId) 
{
	...
	if(lz_chat_data.InputFieldValues[lz_array_indexOf(lz_chat_data.InputFieldIndices,114)].Active)
		lz_chat_data.ExternalUser.Question = lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','').document.getElementsByName('form_114')[0].value.substr(0,64000);
	...
	lz_chat_data.PermittedFrames=3;
...
function lz_chat_activate()
{
    ...
    lz_chat_get_frame_object('','lz_chat_frameset_chat').rows="30,*";
    lz_chat_get_frame_object('lz_chat_frame.3.2.chat.0.0','lz_chat_navigation_table').style.display = '';
    lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_floor').style.display = '';
    lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_main').style.display = '';
    lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_call_me_back_info').style.visibility = 'hidden';
...
function lz_chat_loaded() 
{
	if(lz_chat_data.ComChatVoucherActive != null)
	{
		lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_com_chat_chat_period_spacer').style.display =
		lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_com_chat_chat_period_caption').style.display =
		lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_com_chat_chat_period_value').style.display = (lz_chat_data.ComChatVoucherActive.Expires > 0) ? "" : "none";
		lz_chat_update_com_chat_data(false);
		lz_chat_switch_com_chat_box(true);
	}

    if(lz_chat_data.CurrentApplication=="callback")
        lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_call_me_back_info').style.visibility = 'visible';
    else
        lz_chat_activate();

	lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_transcript_email').value=lz_chat_data.ExternalUser.Email;
	lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_send_chat_transcript').checked = (lz_chat_data.ExternalUser.Email!='');
	lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_transcript_email').disabled = !(lz_chat_data.ExternalUser.Email!='');
	...
	if(lz_chat_data.Status.Status < lz_chat_data.STATUS_STOPPED)
	{
		...
		for(var i in lz_chat_data.ExternalUser.MessagesSent)
		{
			...
			if(lz_chat_data.ExternalUser.MessagesSent[i].MessageTranslation != '')
			{
				...
				_post += "&p_ptiso" + counter.toString() + "=" + lz_global_base64_url_encode(lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_translation_target_language').options[lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_translation_target_language').selectedIndex].value);
			}
			counter++;
		}
		counter = 0;
...
function lz_chat_stop_system()
{
	...
	{
		lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_translation_target_language').disabled =
		lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_translation_service_active').disabled =
		lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_transcript_email').disabled =
		lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_send_chat_transcript').disabled =
	...
	if(lz_chat_data.InputFieldIndices != null)
	{
		...
		for(var i = 0;i < lz_chat_data.InputFieldIndices.length;i++)
		{
			...
			if(_document.getElementById("lz_form_active_" + findex).value == "true" || (findex==116 && lz_chat_data.CallMeBackMode))
			{
                ...
                _document.getElementById("lz_form_" + findex).style.display = (lz_chat_data.InputFieldValues[i].Type == "File" && lz_chat_data.CurrentApplication!="ticket") ? "none" : _document.getElementById("lz_form_" + findex).style.display;
                ...
                if(_document.getElementById("lz_form_info_" + findex).innerHTML.length > 0)
                {
                    ...
                    _document.getElementById("lz_form_mandatory_" + findex).onclick = new Function("parent.parent.lz_chat_show_info_box('"+findex.toString()+"',true);");
                    _document.getElementById("lz_form_info_" + findex).onclick = new Function("parent.parent.lz_chat_show_info_box('"+findex.toString()+"',false);");
                }
                else
...
{
	var group = lz_chat_data.Groups.GetGroupById(lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','lz_form_groups').value);
	if(group == null)
	{
		lz_chat_chat_alert(lz_chat_data.Language.SelectValidGroup,lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0',''),null,null,null);
		return false;	
	}
...
function lz_chat_goto_message(_inChat,_send)
{
    ...
    if(_inChat)
    {
        ...
        lz_chat_data.PermittedFrames = (lz_chat_data.IECompatible) ? 3 : 2;
...
function lz_chat_alternative_ticket_page()
{
    try
    {
        lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','lz_chat_login').style.display='none';
        lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','lz_form_details').style.display='none';
        lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','lz_chat_loading').style.display='';
    }
    catch(e){}
...
function lz_chat_release(_chatPossible, _groupError)
{
	...
	{
        if(lz_small)
            frames['lz_chat_frame.1.1'].document.getElementById('lz_chat_top_bg').style.display = "none";

        document.getElementById("lz_chat_main_frame").rows = (!lz_small ? 99 : 0) + ",*";
        document.getElementById("lz_chat_content_frame").rows = "*,20";
        ...
        if(!_chatPossible || (lz_chat_data.DirectLogin && (!lz_chat_validate_group(true) || !lz_check_missing_inputs(lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0',''),true,false,'lz_chat_check_login_inputs','lz_chat_frame.3.2.lgin.1.0'))))
            lz_chat_data.DirectLogin = false;
...
function lz_check_missing_inputs(_frame,_display,_alert,_contFunc,_frameName)
{
...
function lz_chat_add_update_vouchers_init(_changeHTML)
{
	...
	lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_com_chat_change_voucher').style.display = (lz_chat_data.ComChatVoucherChangeHTML.length > 0) ? "" : "none";
}
...
function lz_chat_add_available_voucher(_id,_chatTime,_chatTimeMax,_chatSessions,_chatSessionsMax,_expires,_expired)
{
	...
	lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_com_chat_change_voucher').style.display = (lz_chat_data.ComChatVouchers.length >= 2) ? "" : "none";
}
...
function lz_chat_check_login_inputs()
{
	if(!lz_chat_validate_group(true))
		return;
	if(!lz_check_missing_inputs(lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0',''),true,true,'lz_chat_check_login_inputs','lz_chat_frame.3.2.lgin.1.0'))
		return;
...
function lz_chat_validate_ticket_inputs()
{
	...
	if(!lz_check_missing_inputs(lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0',''),true,true,'lz_chat_validate_ticket_inputs','lz_chat_frame.3.2.lgin.1.0'))
		return;
	...
	if(lz_chat_data.InputFieldValues[lz_array_indexOf(lz_chat_data.InputFieldIndices,113)].Active)
		lz_chat_data.ExternalUser.Company = lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','').document.getElementsByName("form_113")[0].value.substr(0,255);
	if(lz_chat_data.InputFieldValues[lz_array_indexOf(lz_chat_data.InputFieldIndices,114)].Active)
		lz_chat_data.ExternalUser.MailText = lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','').document.getElementsByName("form_114")[0].value.substr(0,64000);
    if(lz_chat_data.InputFieldValues[lz_array_indexOf(lz_chat_data.InputFieldIndices,116)].Active || lz_chat_data.CallMeBackMode)
		lz_chat_data.ExternalUser.Phone = lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','').document.getElementsByName('form_116')[0].value.substr(0,255);
...
function lz_chat_is_file_upload()
{
    ...
    {
        var findex = lz_chat_data.InputFieldIndices[i];
        if(lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','').document.getElementById("lz_form_" + findex) != null)
            if(!lz_chat_input_is_hidden(findex) && lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','').document.getElementsByName("form_" + findex)[0].tagName.toUpperCase() == "INPUT" && lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','').document.getElementsByName("form_" + findex)[0].type.toUpperCase() == "FILE")
                return true;
    }
...
{
    if(lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0',"lz_form_" + _findex).style.display == "none")
        return true;
    return false;
}
...
function lz_chat_mail_progress(evt)
{
    if (evt.lengthComputable)
    {
        ...
        lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','lz_action_button').value = lz_chat_data.Language.SendMessage + " ("+ percentComplete+ "%)";
    }
}
...
function lz_chat_mail_callback(_success)
{
    ...
    lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','').document.getElementsByName("lz_login_form")[0].reset();
    lz_chat_change_group(lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','lz_form_groups'),false);
    ...
    if(_success)
	{
        ...
        lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','lz_tab_callback').className = "lz_chat_navigation_tab";
	}
	else
	{
...
lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_file_name').value = lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_file_base').value;
}
...
function lz_chat_file_request_upload()
{
	...
	if(!lz_chat_data.FileUpload.Running)
	{
		...
		if(lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_file_name').value.length == 0)
		{
		...
		lz_chat_data.FileUpload.Filename = lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_file_name').value
		lz_chat_data.FileUpload.Permitted = false;
		lz_chat_data.FileUpload.Running = true;
		lz_chat_data.FileUpload.Error = false;
        lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_file_load').style.display = "block";
        lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_file_load').src = "./images/lz_circle.gif?acid=" + lz_global_microstamp();
        lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_file_name').disabled =
        lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_file_select').disabled = true;
        lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_file_send').value = "<!--lang_client_abort-->";
...
function lz_chat_file_start_upload(_file)
{
	try
  	{
        lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_upload_form_userid').value = lz_chat_data.ExternalUser.Session.UserId;
        lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_upload_form_browser').value = lz_chat_data.ExternalUser.Session.BrowserId;
        lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_file_base').value;

        var fd = new FormData(lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','').lz_file_form);
        lz_poll_request	= new lz_connector(lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','').lz_file_form.action,fd,-1);
        lz_poll_request.OnLoadEvent = lz_chat_file_completed;
        lz_poll_request.OnErrorEvent =
        lz_poll_request.OnTimeoutEvent = lz_chat_file_canceled;
        lz_poll_request.OnProgressEvent = lz_chat_file_progress;
        lz_poll_request.ConnectAsync();
	}
	catch(err)
	{
...
function lz_chat_file_progress(evt)
{
    ...
    {
        var percentComplete = Math.round(evt.loaded * 100 / evt.total);
        lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_file_send').value = "<!--lang_client_abort--> (" + percentComplete + "%)";
    }
}

function lz_chat_file_completed(evt)
{
    lz_chat_file_stop(false);
    lz_chat_file_reset();
    eval(evt.target.responseText);
}

function lz_chat_file_canceled(evt)
{
    lz_chat_file_error(lz_chat_data.FILE_UPLOAD_OVERSIZED);
}
...
function lz_chat_file_error(_value)
{
    ...
    lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_file_error').style.display = "block";
}
...
function lz_chat_file_ready()
{
    ...
    lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_file_success').style.display = "block";
...
function lz_chat_file_stop(_error)
{
	if(lz_chat_data.Status.Status < lz_chat_data.STATUS_START || lz_chat_data.Status.Status == lz_chat_data.STATUS_STOPPED)
		return;
    ...
    lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_file_base').value =
    lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_file_name').value = "";
	lz_chat_data.FileUpload.Running = 
	lz_chat_data.FileUpload.Permitted = false;
	lz_chat_data.FileUpload.Error = _error;
	lz_chat_shout(3);
	lz_chat_file_unset_images();

    lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_file_send').disabled = true;
    lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_file_name').disabled =
    lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_file_select').disabled = false;
    lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_file_send').value = "<!--lang_client_send-->";
	setTimeout("lz_chat_file_reactivate();",lz_chat_data.ChatFrequency*2*1000);
}

function lz_chat_file_reactivate()
{
    lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_file_send').disabled = false;
}
...
function lz_chat_file_unset_images()
{
    try
    {
        lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_file_load').style.display =
        lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_file_success').style.display =
        lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0','lz_chat_file_error').style.display = "none";
    }
    catch(err)
    {
...
function lz_chat_tab_set_active(_application, _userActivity)
{
    try
    {
        ...
        if(_userActivity && !lz_chat_data.ChatGroupAvailable)
            ...
            {
                lz_chat_chat_alert(lz_chat_data.Language.ChatNotAvailable,lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0',''),new Function("lz_chat_tab_set_active('ticket',true);"),lz_chat_data.Language.LanguageLeaveMessageShort,null,true);
                return;
            }
        ...
        lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','lz_input_header_box').style.display = (lz_small) ? 'none' : '';
        ...
        if(_application == "ticket")
        {
            ...
            lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','lz_form_details').style.display = "";
            lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','lz_group_selection_box').style.display = (lz_chat_data.HideGroupTicket) ? "none" : "";
        ...
        else
        {
            ...
            lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','lz_group_selection_box').style.display = (lz_chat_data.HideGroupChat) ? "none" : "";
        }
        ...
        lz_chat_get_frame_object('lz_chat_frame.3.2.lgin.1.0','lz_chat_navigation_status').className = (online) ? "lz_chat_navigation_status_online" : "lz_chat_navigation_status_offline";
...
function lz_chat_feedback_switch(_id,_number,_value)
{
    if(lz_chat_rate_comment_sent)
        return;
    if(_value)
        for(var i = 1;i<=_number;i++)
            lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0',"lz_chat_star_" + _id + "_" + i).src = lz_chat_rate_file_1;
    else
        for(var i = 1;i<=5;i++)
            if(i > lz_chat_rate_settings[_id])
                lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0',"lz_chat_star_" + _id + "_" + i).src = lz_chat_rate_file_0;
}

function lz_chat_feedback_set(_id,_number)
{
    if(lz_chat_rate_comment_sent)
        return;
...
function lz_chat_feedback_prepare()
{
    if(lz_chat_rate_comment_sent)
        return;
    if(!lz_chat_rate_comment_started)
        lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0',"lz_chat_rate_comment").value="";
    lz_chat_rate_comment_started=true;
}

function lz_chat_feedback_validate()
{
    if(lz_chat_rate_settings[0] == 0 || lz_chat_rate_settings[1] == 0)
    {
        lz_chat_chat_alert(lz_chat_data.Language.SelectRating,lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0',''),null);
        return;
    }
    if(!lz_chat_rate_comment_started)
        lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0',"lz_chat_rate_comment").value = "";
    if(lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0',"lz_chat_rate_comment").value.length > 400)
    {
        lz_chat_chat_alert(lz_chat_data.Language.MessageTooLong + " (-" + (lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0',"lz_chat_rate_comment").value.length-400).toString() + ")",lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0',''),null);
        return;
    }
    lz_chat_rate_comment_sent =
    lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0',"lz_chat_rate_comment").disabled =
    lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0',"lz_chat_rate_button").disabled = true;
    lz_chat_send_rate(lz_chat_rate_settings[0],lz_chat_rate_settings[1],lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0',"lz_chat_rate_comment").value);
}

function lz_chat_feedback_callback(_message)
{
    lz_chat_switch_feedback();
    lz_chat_chat_alert(_message,lz_chat_get_frame_object('lz_chat_frame.3.2.chat.5.0',''),null);
}
