...
{
	var getValues = "&browid="+lz_global_base64_url_encode(lz_session.BrowserId)+"&ue="+lz_global_base64_url_encode(lz_global_base64_url_encode(window.location.href))+"&pc="+(++lz_poll_id);
	...
	getValues += "&el="+lz_user_language+"&code="+lz_area_code+"&dc="+lz_global_base64_url_encode(document.title);
    ...
    if(lz_deactivate != null)getValues += "&deactr=" + lz_deactivate;
...
function lz_tracking_action_result(_action,_result,_closeOnClick,_parameters)
{
	...
	_parameters = "&browid="+lz_global_base64_url_encode(lz_session.BrowserId)+"&ue="+lz_global_base64_url_encode(window.location.href) + _parameters;
...
function lz_tracking_send_alert(_alertId,_text)
{
	if(lz_alert_active == null && lz_overlays_possible)
	{
		...
		lz_alert_window = new lz_livebox("lz_alert_window",(lz_global_base64_decode(lz_alert_html)),400,400,0,0,0,0,11,1,0);
