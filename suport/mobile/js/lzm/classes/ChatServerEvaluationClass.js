    ...
    $(xmlDoc).find('login').each(function () {
        ...
        login.children('login_return').each(function () {
            ...
            for (var attrIndex = 0; attrIndex < myAttributes.length; attrIndex++) {
                ...
                if (thisClass.oo == null && myAttributes[attrIndex].nodeName == 'oo') {
                    thisClass.oo = lz_global_base64_url_decode(myAttributes[attrIndex].nodeValue);
                }
            }
    ...
    if (this.crc3 == null) {
        try {
            ...
            if (this.crc3 != null) {
                ...
                for (i=1; i<=5; i++) {
                    if (lzm_chatTimeStamp.getServerTimeString(null, true) - parseInt(this.crc3[0]) > 1209600 && this.crc3[i] == '0') {
                        this.crc3[i] = '-2';
                    }
                }
                if ((lzm_chatTimeStamp.getServerTimeString(null, true) - parseInt(this.crc3[0]) > 1209600 && this.crc3[2] == '0') || this.crc3[2] == '-2') {
                    this.crc3[6] = '';
                }
    ...
    $(xmlDoc).find('ext_f').each(function () {
        ...
        $(ext_f).find('fw').each(function () {
            ...
            if ($.inArray(new_forward.id, thisClass.extForwardIdList) == -1) {
                ...
                var extUserName = '';
                var visitor = thisClass.visitors.getVisitor(extUserId);
                if (visitor != null) {
                    for (var browserIndex=0; browserIndex<visitor.b.length; browserIndex++) {
                        if (visitor.b[browserIndex].id == extUserBId) {
...
ChatServerEvaluationClass.prototype.addExtUserV = function (v) {
    ...
    $(v).find('rv').each(function() {
        ...
        for (var attrIndex = 0; attrIndex < myAttributes.length; attrIndex++) {
            new_rv[myAttributes[attrIndex].nodeName] = lz_global_base64_url_decode(myAttributes[attrIndex].nodeValue);
        }
        new_rv.b = [];
        ...
        for (i=0; i<visitor.b.length; i++) {
            ...
            try {
                if (typeof visitor.b[i].chat != 'undefined' && visitor.b[i].chat.id != '') {
                    ...
                    if (/*(visitor.b[i].chat.at == 0 || (visitor.b[i].chat.at * 1000) > thisClass.loginTime) &&
                     */$.inArray(thisClass.myId, visitor.b[i].chat.pn.memberIdList) != -1 &&
                        chatNotDeclined) {
                        if (thisUserChat == null) {
                            thisClass.userChats.setUserChat(visitor.id + '~' + visitor.b[i].id, {status: 'new', type: 'external',
                                id: visitor.id, b_id: visitor.b[i].id, group_chat: false});
            ...
            // Remove visitor/browser from open chat or add op has left messages
            try {
                if (thisUserChat != null && typeof visitor.b[i].chat.pn != 'undefined' &&
...
ChatServerEvaluationClass.prototype.getReports = function(xmlDoc) {
    var that = this, reports = [];
    $(xmlDoc).find('dr').each(function() {
        var dr = $(this);
        reports.push(that.addReport(dr));
    });
};

ChatServerEvaluationClass.prototype.addReport = function(dr) {
...
ChatServerEvaluationClass.prototype.getDepartments = function (xmlDoc) {
    ...
    $(xmlDoc).find('int_d').each(function () {
        ...
        $(int_d).find('v').each(function () {
            try {
                var v = $(this);
                var newGroup = thisClass.addDepartment(v);
...
ChatServerEvaluationClass.prototype.getInternalUsers = function (xmlDoc) {
    ...
    $(xmlDoc).find('int_r').each(function () {
        ...
        $(int_r).find('v').each(function () {
            var v = $(this);
            var newOperator = thisClass.addInternalUser(v);
...
ChatServerEvaluationClass.prototype.escapeHtml = function(myText) {
    ...
    // Replace & by &amp; to properly display html entities
...
ChatServerEvaluationClass.prototype.addDepartment = function (v) {
    ...
    if (typeof new_department.id != 'undefined') {
        ...
        new_department.name = (typeof new_department.humanReadableDescription[thisClass.userLanguage] != 'undefined') ?
            ...
            (typeof new_department.humanReadableDescription[thisClass.defaultLanguage] != 'undefined') ?
    ...
    $(v).find('crm').each(function() {
        ...
        for (var attrIndex = 0; attrIndex < myAttributes.length; attrIndex++) {
            newCrm[myAttributes[attrIndex].nodeName] = lz_global_base64_url_decode(myAttributes[attrIndex].nodeValue)
        }
