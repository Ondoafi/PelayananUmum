        ...
        if (sel != null) {
            try {
                ...
                if(ibody.lastChild && ibody.lastChild != null)
                {
                    ...
                    {
                        //console.log(caret + "/" + ibody.lastChild.tagName + "/" + ibody.lastChild.nodeType);
                        //console.log(ex);
                    }
                    sel.removeAllRanges();
                    sel.addRange(range);
...
var iframe = '<table class="lzmWyzzEditor" id="wysiwyg' + n + 'table" cellpadding="0" cellspacing="0" border="0" style="width:' + wyzzW + m_SizeUnitWidth+ '; height:' + wyzzH + m_SizeUnitHeight+ ';border: 0px inset #dddddd;">' +
    '<tr><td valign="top" style="width:' + wyzzW + m_SizeUnitWidth+ '; height:' + wyzzH + m_SizeUnitHeight+ ';">\n'
  ...
  doc.close();
    //  onclick="console.log(\'Internal click\');"
    doc.body.ontouchstart = function(e) {
	...
	{
        doc.addEventListener("keypress", window.parent.catchInputs, false);
        doc.addEventListener("paste", window.parent.catchPaste, false);
		//doc.addEventListener("ondragover", window.parent.catchDragOver, false);
    } 
	else if (doc.attachEvent) 
	{
        doc.attachEvent('onkeypress', window.parent.catchInputs);
        doc.attachEvent('onpaste', window.parent.catchPaste);
		//doc.attachEvent('ondragover', window.parent.catchDragOver);
    }
...
function catchPaste(e) 
{
    try {
        if (!showHtml)
        {
            setTimeout(function()
            {
                var foo = document.getElementById('wysiwyg' + wyzzId).contentWindow.document.body.innerHTML;
                foo = foo.replace(/&lt;.*?&gt;/g,'');
                var regex = /(<([^>]+)>)/ig;
                var result = foo.replace(regex, "");
                document.getElementById('wysiwyg' + wyzzId).contentWindow.document.body.innerHTML = result;
                lz_he_setCursor(wyzzId);
            },0);
        }
    } catch(ex) {
        // Do nothing...
