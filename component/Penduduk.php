...
class Penduduk
{
    ...
    function login($id,$nama,$noKTP,$tempatLahir,$tglLahir,
                   $alamat, $rt, $rw, $kelurahan, $kecematan, $kelamin, 
                   $kAgama, $agama, $kKerja, $pekerjaan, $username,$password)
	{ 
		$statement = "
		SELECT 
			id, nama, password, no_ktp, tempat_lahir, tgl_lahir, 
            alamat, rt, rw, kelurahan,kecamatan, kelamin, k_agama, 
            agama, k_kerja, pekerjaan,is_admin
		FROM 
			penduduk 
		...
		list($this->id, $this->nama, $pass, $this->noKTP,$this->tempatLahir,
             $this->tglLahir, $this->alamat, $this->rt, $this->rw,
             $this->kelurahan, $this->kecamatan, $this->kelamin, 
             $this->kAgama, $this->agama, $this->kKerja, $this->pekerjaan)= $result->elementAt(0);
        if($pass === md5($password)){
			$_SESSION["penduduk"]["id"]          = $this->id;
			$_SESSION["penduduk"]["username"]    = $username;
			$_SESSION["penduduk"]["nama"]        = $this->nama;
            $_SESSION["penduduk"]["no_ktp"]      = $this->noKTP ;
            $_SESSION["penduduk"]["tempat_lahir"]= $this->tempatLahir;
            $_SESSION["penduduk"]["tgl_lahir"]   = $this->tglLahir ;
            $_SESSION["penduduk"]["alamat"]      = $this->alamat ;
            $_SESSION["penduduk"]["rt"]          = $this->rt;   
            $_SESSION["penduduk"]["rw"]          = $this->rw;
            $_SESSION["penduduk"]["kelurahan"]   = $this->kelurahan;
            $_SESSION["penduduk"]["kecamatan"]   = $this->kecamatan;
            $_SESSION["penduduk"]["kelamin"]     = $this->kelamin;
            $_SESSION["penduduk"]["k_agama"]      = $this->kAgama;
            $_SESSION["penduduk"]["agama"]       = $this->agama;
            $_SESSION["penduduk"]["k_kerja"]     = $this->kKerja;
            $_SESSION["penduduk"]["pekerjaan"]   = $this->pekerjaan;
            $_SESSION["penduduk"]["is_admin"]    = $this->isAdmin;
    ...
    function logout()
	{
        ...
        //session_unregister('penduduk');
		 unset($_SESSION["penduduk"]);
	}
